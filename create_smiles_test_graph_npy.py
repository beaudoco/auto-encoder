...
class SparseMolecularTestDataSet():
    ...
    def _generate_AX(self):
        ...
        for i, mol in enumerate(self.data):
            ...
            if A is not None:
                ...
                smiles_file = open('USPTO-50K/tgt-test.txt', 'r')
                ...
                for idx in range(len(chunks_file)):
                    ...
                    if chunks_file[idx] == tmpMol:
                        src_file_exists = os.path.exists("USPTO-50K-IMAGES-SRC-TEST/mol-{0}.npy".format(idx)) 
                        tgt_file_exists = os.path.exists("USPTO-50K-IMAGES-TGT-TEST/mol-{0}.npy".format(idx)) 
                        if src_file_exists:
                            os.remove("USPTO-50K-IMAGES-SRC-TEST/mol-{0}.npy".format(idx))
                        if tgt_file_exists:
                            os.remove("USPTO-50K-IMAGES-TGT-TEST/mol-{0}.npy".format(idx))
                        break
                smiles_file.close()
...
if __name__ == '__main__':
    data = SparseMolecularTestDataSet()

    data.generate(filters=lambda x: x.GetNumAtoms() <= 100)
    data.save('tgt_test.sparsedataset')
